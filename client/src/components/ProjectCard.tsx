  ...
  return (
    ...
    hover:border-white/20 transition group'>
        ...
        <div className={`${gen?.aspectRatio === '9:16' ? 'aspect-9/16' : 'aspect-video'} relative overflow-hidden`}>
          ...
          {gen.generatedVideo && (
            <video src={gen.generatedVideo} muted loop playsInline className='absolute insert-0 w-full 
         h-full object-cover opacity-0 group-hover:opacity-100 transition duration-500'
              onMouseEnter={(e) => e.currentTarget.play()}
              onMouseLeave={(e) => e.currentTarget.pause()} />
          )}

          {(!gen?.generatedImage && !gen?.generatedVideo) && (
            <div className='absolute inset-0 w-full h-fill flex flex-col items-center justify-center bg-black/20'>
         ...
         verflow-hidden right-0 peer-focus:block hover:black w-40 bg-black/50 backdrop-blur text-white
         border border-gray-500/50 rounded-lg shadow-md mt-2 py-1 z-10`}>
          ...
          <div className='absolute right-3 bottom-3'>
            <img src={gen.uploadedImages[0]} alt="product" className='w-16 h-16 object-cover rounded-full aniamte-float' />
            <img src={gen.uploadedImages[1]} alt="model" className='w-16 h-16 object-cover rounded-full aniamte-float ml-8' style={{ animationDelay: '3s' }} />
          </div>
